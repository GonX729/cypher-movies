...
api_key = os.environ.get('86dd6f44c076b7edaf46d6028977586a')
PORT = int(os.environ.get('PORT', '8443'))
logger = logging.getLogger(__name__)

TOKEN = os.environ.get('7874806559:AAGsljHpS_lAScGIn48SnV7bfaZHuo7V9Vs')
name = ''
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        'Welcome To  Movie CypherXelite Hub the place where movie links meet!..../usage if you are having issues.')


async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Check if the user belongs to the telegram group"""
    chat = context.bot
    chat = chat.getChatMember(user_id=update.effective_user.id, chat_id='@CypherXelites')
    result = await chat
    if result.status == 'left':
        await update.message.reply_text('You have to be on the telegram channel first!')
        await update.message.reply_text(parse_mode='HTML', text='https://t.me/cypherXelites')
